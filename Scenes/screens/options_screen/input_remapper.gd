...
func _update_text() -> void:
	...
	for e in InputMap.action_get_events(action):
		...
		if profile == Profile.KEYBOARD_AND_MOUSE and not Controls._is_event_keyboard_and_mouse(e):
			continue
		
		elif profile == Profile.CONTROLLER and not Controls._is_event_controller(e):
			continue
	
		if count > 0:
			text += ", "
		
		text += e.as_text() if profile == Profile.KEYBOARD_AND_MOUSE else Controls.convert_controller_to_string(e)
...
func _input(event: InputEvent) -> void:
	...
	if profile == Profile.KEYBOARD_AND_MOUSE and Controls._is_event_keyboard_and_mouse(event):
		_input_keyboard_and_mouse(event)
	
	elif profile == Profile.CONTROLLER and Controls._is_event_controller(event):
...
func _input_keyboard_and_mouse(event: InputEvent) -> void:
	...
	for e in InputMap.action_get_events(action):
		if Controls._is_event_keyboard_and_mouse(e):
...
func _input_controller(event: InputEvent) -> void:
	...
	for e in InputMap.action_get_events(action):
		if Controls._is_event_controller(e):
			InputMap.action_erase_event(action, e)
